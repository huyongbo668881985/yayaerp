...
CREATE TABLE IF NOT EXISTS platform_admins (
  ...
  name TEXT NOT NULL,
  -- 强制改密标记：首次启动自动创建的默认超管（superadmin/super123）置 1，
...
// 首次启动生成默认平台超管账号。
// 默认口令是写在代码里的公开值，必须先强制改密再放行其它后台页面，
// 否则部署到公网后没来得及改密码的窗口期就是裸奔。
const adminCount = platformDb.prepare('SELECT COUNT(*) AS c FROM platform_admins').get().c;
if (adminCount === 0) {
  const hash = bcrypt.hashSync('super123', 10);
  platformDb.prepare(
    'INSERT INTO platform_admins (username, password_hash, name, must_change_password) VALUES (?,?,?,1)'
  ).run('superadmin', hash, '平台超级管理员');
  console.log('已创建默认平台超管账号: superadmin / super123 —— 首次登录会被强制要求修改密码');
}
...
function updatePlatformAdminPassword(id, newPasswordHash) {
  platformDb.prepare(
    'UPDATE platform_admins SET password_hash = ?, must_change_password = 0 WHERE id = ?'
