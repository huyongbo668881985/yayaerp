 ...
 *          到 Cloudflare R2（未配置 R2_* 环境变量时自动跳过，只做本地备份）。
...
const { syncBackupsToR2, isR2Configured } = require('./r2Sync');
...
/**
 * 执行一次完整备份任务：校验密钥 -> 逐库快照（含加密+校验） -> 汇总结果
 *                    -> 清理过期快照 -> rclone 同步 R2（未配置则跳过）。
 * 单个库失败不中断其他库（继续备剩下的），所有错误汇总在返回值里。
 */
async function runBackup(options = {}) {
  // 密钥检查放在最前面 fail fast：不生成任何半成品/未加密产物
