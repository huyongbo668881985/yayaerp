...
const rawTimeout = process.env.R2_SYNC_TIMEOUT_MS;
const SYNC_TIMEOUT_MS = rawTimeout == null || rawTimeout === '' ? 15 * 60 * 1000 : Number(rawTimeout);
if (!Number.isInteger(SYNC_TIMEOUT_MS) || SYNC_TIMEOUT_MS <= 0) {
  throw new Error(`R2_SYNC_TIMEOUT_MS 必须是大于 0 的整数毫秒，当前值无效：${rawTimeout}`);
}
