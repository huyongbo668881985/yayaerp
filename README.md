...
- 首次启动自动创建平台超管：`superadmin / super123`。默认口令写在代码里、等于公开的，所以**首次登录会被强制跳转到改密页，不改密码进不去后台任何其它页面**（改完密码标记才清除，之后用新密码正常登录）
...
```bash
# 全量上线前回归（spawn 真实服务进程，覆盖 Web 全功能）
npm test

# 快速自检：2026-09-10 修复的三个守卫点（成本价不外泄 / 反审核不重复加库存 / 默认超管强制改密）
# 同进程起服务、跑完自动清理临时数据目录，几十秒内出结果
node tests/p1-fixes-verify.js
```

## 九、后续可扩展方向（现在没做，等真的需要再加）
